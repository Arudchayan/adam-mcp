...
**Status:** Product constitution is `docs/product-spec.md`. Trust gate is `SECURITY.md`. Live ADAM is a dedicated Chrome-session provider (`ADAM_PROVIDER=browser` or `--browser`). SOAP and HTML providers fail closed (SOAP is Apache 403; not a SWITCH login). Default MCP provider is `fixture`. Do not paste SWITCH edu-ID credentials into chat.
