...
const TOKENISH_QUERY =
  /([?&](?:token|access_token|session|sid|phpsessid|iliassessid|jwt|php?sessid)=)[^&]+/gi;
...
/** Bare ILIAS session cookie assignment (not only Cookie: header lines). */
const ILIASSESSID = /iliassessid\s*[:=]\s*[^\s;]+/gi;
...
export function redactText(value: string): string {
  return value
    ...
    .replace(ILIASSESSID, "ILIASSESSID=[redacted]")
...
export function redactUrl(value: string): string {
  try {
    ...
    for (const key of [...url.searchParams.keys()]) {
      if (
        /token|session|sid|jwt|cookie|phpsessid|iliassessid|saml|relaystate|shib|password|api[_-]?key|secret/i.test(
          key,
        )
      ) {
