...
| ID | Threat | Control |
| --- | --- | --- |
| T1 | Model uses the student’s ADAM session as a deputy | Read-only tools, interim `confirm: true` schema gate on page/extract bodies and forum post bodies (ADR 0012; auto-fillable until MCP elicitation), deny `tst`, no generic fetch |
