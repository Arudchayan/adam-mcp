...
| ID | Threat | Control |
| --- | --- | --- |
| T1 | Model uses the student’s ADAM session as a deputy | Read-only tools, confirm on page bodies, deny `tst`, no generic fetch |
...
| T4 | Cookie leak via tools/logs | Cookies omitted from tools and status |
