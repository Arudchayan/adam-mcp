...
Scanned PDFs fail with `unsupported_type` (no OCR). Empty or whitespace extracts fail closed rather than returning a blank success (ADR 0014). `confirm: true` is an interim schema gate — not a host elicitation modal, not an OS permission, and not equated to tool annotations alone. Real confirms / future writes → elicitation when the host supports MRTR. Extracted text is wrapped as untrusted data.
